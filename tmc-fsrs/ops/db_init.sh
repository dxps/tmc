...
# Check if a custom user has been set, otherwise default to 'tmc'.
DB_USER="${POSTGRES_USER:=tmc}"
# Check if a custom password has been set, otherwise default to 'tmc'.
DB_PASSWORD="${POSTGRES_PASSWORD:=tmc}"
# Check if a custom password has been set, otherwise default to 'tmc'.
DB_NAME="${POSTGRES_DB:=tmc}"
# Check if a custom port has been set, otherwise default to '5442'.
...
then
  ...
  RUNNING_POSTGRES_CONTAINER=$(docker ps --filter 'name=tmc' --format '{{.ID}}')
